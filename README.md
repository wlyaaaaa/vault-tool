...
| **管理** | ⬆️ 升级旧版到 VAULT03（仅旧库出现）・ 🔑 修改密码 / 密钥文件 ・ 📋 查看库信息（不需密码） |
...
```bash
python test_vault_tool.py            # 68 个单元/集成测试
# 或： python -m pytest test_vault_tool.py -v
```

覆盖：密钥派生、V1/V2/V3 往返、篡改检测、路径遍历防护、密钥文件、诱饵双层、
压缩（含 gzip mtime 去元数据）、隐写、安全删除、内存清零、密码强度；以及**交互流程**
（加密 / 解密-解压 / 合并添加文件 / 改密码 / 升级 / 设诱饵）和**持久菜单循环**
（异常、密码锁定、Ctrl+C 都回菜单，仅 [0]/EOF 退出）——用快速 KDF 替身保证测试秒级完成。
