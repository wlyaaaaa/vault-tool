...
| 只看支持的文本 | `decrypt --no-disk` | 默认在本机内存里查看，不主动提取原文文件。 |
| 用外部程序打开附件 | `decrypt --extract` | 按名称筛选后提取到 `decrypted/`，使用结束清理；强杀可能残留。 |
...
- 提取可按名称筛选；声明总解压量超过 4 GiB 会询问是否继续，并非所有操作都被硬限为 4 GiB。
