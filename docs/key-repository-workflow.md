...
1. 把需要备份的明文放进 `E:\Vault\source\`，或用脚本参数复制进去。
2. 在本地可见终端运行：

   ```powershell
   E:\Vault\scripts\Start-KeyVaultEncrypt.ps1
   ```

3. 只在本地终端密码提示里输入保险库密码。不要把密码发到聊天、命令行参数、脚本、日志或 Git。
4. 生成 `E:\Vault\vault.enc` 后，先本地解密演练一次。
5. 确认可恢复后，用 GitHub API 上传密文到私有 `Key` 仓库：

   ```powershell
   E:\Vault\scripts\Publish-KeyVaultToGitHub.ps1
   ```
